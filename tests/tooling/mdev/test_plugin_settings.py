...
class OnlyWhatAPluginCanReadBack(unittest.TestCase):
    ...
    def test_an_oversized_integer_refusal_prints_no_digits(self) -> None:
        """An all-digit secret -- a session id, an account number -- is
        exactly the shape that reaches this refusal, so it names the length
        and not the number. It used to print the value AND the value the
        plugin would have seen, which leaks it twice.

        400 digits exceeds the float range; 5000 exceeds Python's own
        integer-string limit and used to raise ValueError one layer earlier,
        where `cli.main` does not catch it and the user got a traceback.
        """
        for value in ("9" * 400, "-" + "9" * 400, "9" * 5000,
                      "12345678901234567"):
            with self.subTest(digits=len(value.lstrip("-"))):
                with self.assertRaises(MdevError) as caught:
                    harness.parse_plugin_setting("P:g:cookie=%s" % value)
                message = str(caught.exception)
                self.assertNotIn(value.lstrip("-")[:20], message)
...
class TheManifestIsReadTheWayTheCoreReadsIt(unittest.TestCase):
    ...
    def test_the_core_decoder_is_still_the_one_described(self) -> None:
        """Asserted, not trusted. The mirror reproduces a decoder; if that
        decoder moves, the mirror is wrong and nothing else would say so.
        The negative half is the one that matters: `- 'F' + 10` was the
        defect (movian#250) and must not come back."""
        decoder = (REPO_ROOT / "src" / "misc" / "json.c").read_text(
            encoding="utf-8")
        self.assertIn("v |= *s - 'A' + 10;", decoder)
        self.assertNotIn("v |= *s - 'F' + 10;", decoder)
    ...
    def test_an_escaped_id_KEY_does_not_hide_the_escape(self) -> None:
        ...
        with self.assertRaises(MdevError) as caught:
            harness.seed_plugin_settings(
                root / "persistent", [str(weird)], ["P\ufffe:g:k=1"])
        self.assertIn("str.c:687-688", str(caught.exception))
        self.assertFalse((root / "persistent").exists())
    ...
    def _manifest(self, root: Path, name: str, text: str) -> str:
        where = root / "src" / name
        where.mkdir(parents=True)
        (where / "plugin.json").write_text(text, encoding="utf-8")
        return str(where)

    def test_the_mirror_matches_the_compiled_core(self) -> None:
        """Measured against a build of this tree's C, by running
        `htsmsg_json_deserialize2` rather than modelling it. A mirror that has
        never been compared to the thing it mirrors is a guess with more
        lines.

        The uppercase row read `PE` until movian#250 was fixed in the core
        (a366d1886). It reads `PJ` now, which is the whole point of that fix
        and the reason the refusal it needed is gone.
        """
        backslash = chr(92)
        for spelling, core in (("PJ", "PJ"),
                               ("P" + backslash + "u004a", "PJ"),
                               ("P" + backslash + "u004A", "PJ"),
                               ("P" + backslash + "u0031FACE", "P1FACE")):
            with self.subTest(spelling):
                text = '{"id":"%s","type":"ecmascript"}' % spelling
                raw = harness._top_level_id_literal(text)
                self.assertEqual(harness._core_decode(raw), core)

    def test_a_literal_hex_suffix_is_not_part_of_the_escape(self) -> None:
        """`P\\u0031FACE` decodes to `P1FACE` on BOTH sides. An unbounded
        `\\u[0-9a-fA-F]*[A-F]` read the literal `FACE` as escape digits and
        refused a plugin nothing was wrong with -- the over-refusal the
        controls were supposed to catch, missed because every control used a
        four-digit escape with nothing after it."""
        tmp = tempfile.TemporaryDirectory()
        self.addCleanup(tmp.cleanup)
        root = Path(tmp.name)
        plugin = self._manifest(
            root, "hexsuffix",
            '{"id":"P%su0031FACE","type":"ecmascript"}' % chr(92))
        self.assertEqual(harness.plugin_manifest_id(plugin), "P1FACE")
        harness.seed_plugin_settings(
            root / "persistent", [plugin], ["P1FACE:g:k=1"])
        self.assertEqual(
            json.loads(harness.plugin_setting_path(
                root / "persistent", "P1FACE", "g").read_text()), {"k": 1})

    def test_a_nested_id_is_not_mistaken_for_the_manifests(self) -> None:
        """A regex has no notion of depth, so `{"meta":{"id":"nested"},...}`
        handed the check a string that names no plugin -- and the real id
        then went uninspected, which is the permissive direction."""
        tmp = tempfile.TemporaryDirectory()
        self.addCleanup(tmp.cleanup)
        root = Path(tmp.name)
        # The directory is NOT named after the nested value, so finding that
        # value in the message can only mean the check read it.
        plugin = self._manifest(root, "deep", json.dumps(
            {"meta": {"id": "innerid"}, "id": "P\U0001F600",
             "type": "ecmascript"}))
        with self.assertRaises(MdevError) as caught:
            harness.plugin_manifest_id(plugin)
        self.assertIn("str.c:687-688", str(caught.exception))
        self.assertNotIn("innerid", str(caught.exception))

    def test_a_nested_id_after_a_nested_pair_is_still_not_the_manifests(
            self) -> None:
        """The shape that needs the depth rule in BOTH places.

        The locator ignores strings below depth 1 and resets its key/value
        turn only on a depth-1 comma. Either alone survives this manifest,
        because each covers the other: disable the string rule and the
        depth-1 comma resynchronises; disable the comma rule and the nested
        strings were never looked at. Disable BOTH and the inner `"id"` is
        taken for the manifest's, which returns `BAD`.

        So this is a joint pin and is reported as one. Neither half is
        individually falsifiable against a manifest Python will parse, and
        the honest reading is that they are one rule -- look only at depth
        1 -- written where each of the two token kinds occurs.
        """
        tmp = tempfile.TemporaryDirectory()
        self.addCleanup(tmp.cleanup)
        root = Path(tmp.name)
        text = json.dumps(
            {"a": {"x": "y", "id": "BAD"}, "id": "P", "type": "ecmascript"})
        self.assertEqual(harness._top_level_id_literal(text), "P")
        plugin = self._manifest(root, "deep3", text)
        self.assertEqual(harness.plugin_manifest_id(plugin), "P")

    def test_a_nested_id_does_not_block_a_sound_manifest(self) -> None:
        tmp = tempfile.TemporaryDirectory()
        self.addCleanup(tmp.cleanup)
        root = Path(tmp.name)
        plugin = self._manifest(root, "deep2", json.dumps(
            {"meta": {"id": "innerid"}, "id": "P", "type": "ecmascript"}))
        self.assertEqual(harness.plugin_manifest_id(plugin), "P")

    def test_the_noncharacters_the_core_drops_are_refused(self) -> None:
        """`utf8_put` writes nothing for U+FFFE and U+FFFF either, not only
        for surrogates -- str.c:687-688 lists all three in one condition, and
        a pattern written for surrogates alone covered one third of it."""
        tmp = tempfile.TemporaryDirectory()
        self.addCleanup(tmp.cleanup)
        root = Path(tmp.name)
        for index, char in enumerate(("\ufffe", "\uffff")):
            with self.subTest(hex(ord(char))):
                plugin = self._manifest(
                    root, "nonchar%d" % index,
                    json.dumps({"id": "P" + char, "type": "ecmascript"}))
                with self.assertRaises(MdevError) as caught:
                    harness.plugin_manifest_id(plugin)
                self.assertIn("U+FFFE", str(caught.exception))
