...
def parse_plugin_setting(spec: str) -> PluginSetting:
    ...
    return PluginSetting(plugin_id, group, key,
                         _coerce_setting(plugin_id, group, key, value))
...
def _coerce_setting(plugin_id: str, group: str, key: str,
                    value: str) -> Any:
    ...
    try:
        ...
        # `coerce_scalar` is shared with --dev-flags and cannot name a
        # setting, so the naming is added here where the parts are known.
        raise MdevError("--plugin-setting %s:%s:%s: %s"
                        % (plugin_id, group, key, error))
    if isinstance(coerced, int) and abs(coerced) > MAX_EXACT_SETTING_INT:
        # Neither the value nor its projection. An all-digit secret -- a
        # session id, an account number -- is exactly the shape that lands
        # here, and printing what the plugin WOULD have seen leaks it just as
        # completely as printing what was typed.
        raise MdevError(
            "--plugin-setting %s:%s:%s has an unquoted integer of %d digits, "
            "outside the range a plugin can read back exactly "
            "(+/-%d, Number.MAX_SAFE_INTEGER): store.js parses the seed with "
            "JSON.parse and a Duktape Number is a double, so the plugin would "
            "see a different number. Quote it to seed the string."
            % (plugin_id, group, key, len(value.lstrip("-")),
               MAX_EXACT_SETTING_INT))
    return coerced
...
# How the core reads a manifest string, mirrored so the two can be compared.
#
# Three separate patterns used to approximate this -- uppercase hex, surrogate
# escapes, a raw search for the `"id"` member -- and each one was a guess
# about the decoder rather than the decoder. Every guess was wrong in its own
# direction: the member search found a NESTED `id` first, the surrogate
# pattern missed U+FFFE and U+FFFF, and the uppercase pattern had an unbounded
# `*` that read the literal `FACE` in `P\u0031FACE` as part of the escape and
# refused an id both sides agree on. Approximating is the defect; this
# reproduces the decoder instead, and the rule becomes one comparison.
#
# `json_parse_string` (json.c:104-130) pushes EVERY character through
# `json_str_read_char` and then `utf8_put`, escaped or not. So the two places
# the core can disagree with Python are both here:
#
#   str.c:687-688   `utf8_put` writes nothing for U+FFFE, U+FFFF or anything
#                   in D800..DFFF, which is deliberate and will not change.
#
# There was a second one until movian#250 landed: json.c decoded uppercase hex
# five too low, so `"P\u004A"` was `PJ` here and `PE` there. That is fixed in
# the core (a366d1886), so this mirror no longer carries it and the refusal
# that named it is gone. What remains is the drop rule, which is not a defect
# and will not be fixed away.
_CORE_SIMPLE_ESCAPES = {"b": 0x08, "f": 0x0C, "n": 0x0A, "r": 0x0D,
                        "t": 0x09}


def _core_decode(raw: str) -> str:
    """What the core builds from the inner text of a JSON string literal.
    ...
    while i < len(raw):
        ...
        if char != "\\":
            ...
            if escape == "u":
                ...
                # `default: return *s` (json.c:53) -- an unknown escape is
                # the character itself.
                value = _CORE_SIMPLE_ESCAPES.get(escape, ord(escape))
        if value in (0xFFFE, 0xFFFF) or 0xD800 <= value < 0xE000:
            continue  # utf8_put writes nothing for these
        out.append(chr(value))
    return "".join(out)


def _string_literal_at(text: str, start: int) -> tuple[str | None, int]:
    """The inner text of the JSON string at `start`, and what follows it."""
    index = start + 1
...
def _top_level_id_literal(text: str) -> str | None:
    """The raw spelling of the TOP-LEVEL `id` member's value.

    Depth matters and a regex has no notion of it: `{"meta":{"id":"nested"},
    "id":"..."}` handed the guards the nested value, so they inspected a
    string that names no plugin. Keys are compared after decoding, because
    `"\u0069d"` is the same member as `"id"` and a manifest may spell it
    either way.

    Returns None when the top-level id is absent or is not a string;
    `_manifest_id` has already refused that on the decoded manifest.
    """
    depth = 0
    index = 0
    ...
    while index < len(text):
        char = text[index]
        if char == '"':
            literal, index = _string_literal_at(text, index)
            if literal is None:
                return None
            if depth == 1:
                if expect_key:
...
def _decoded_key(raw: str) -> str | None:
    try:
...
def _manifest_id(plugin_dir: str, manifest: dict) -> str:
    ...
    raw_id = _top_level_id_literal(
        manifest_path.read_text(encoding="utf-8"))
    if raw_id is not None:
        as_core_reads_it = _core_decode(raw_id)
        if as_core_reads_it != value:
            raise MdevError(
                "%s declares the id %r, which the core reads as %r. Seeding "
                "it would write a profile the core never creates and the "
                "plugin would read nothing. The cause is a code point "
                "utf8_put writes nothing for -- U+FFFE, U+FFFF, or anything "
                "in D800..DFFF, which is what an escaped surrogate PAIR is "
                "made of (str.c:687-688). That is deliberate and will not "
                "change." % (manifest_path, value, as_core_reads_it))
    return value
...
def coerce_scalar(value: str) -> Any:
    ...
    `int()` refuses a decimal string past `sys.get_int_max_str_digits()` --
    4300 by default since 3.11 -- and raises ValueError, which `cli.main`
    does not catch. So a long enough run of digits produced a traceback from
    the code whose job is a clean refusal, one layer above the safe-integer
    guard that would have refused it anyway.
    """
    ...
    try:
        ...
        raise MdevError(
            "a value of %d digits cannot be read as a number at all "
            "(Python's integer-string limit). A plugin could not hold it "
            "either: store.js parses with JSON.parse and a Duktape Number "
            "is a double. Quote it to seed the string."
            % len(value.lstrip("-")))
