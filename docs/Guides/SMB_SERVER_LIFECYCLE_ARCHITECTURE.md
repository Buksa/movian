...
* Каждый открытый файл резервирует свободный слот в массиве `sc_files` (размером до `SMB2_MAX_FILES` = 256). Хендлы сопоставляются через кодируемый 16-байтовый PDU File ID.
