...
* **`smb_tree_connect`**: Монтирует шару. Проверяет имя ресурса. Разрешает монтирование только при совпадении имени шары с `smb_share_name`.
...
* **`smb_query_info`**: Запрашивает метаданные о файле/директории. Возвращает атрибуты (`FILE_ATTRIBUTE_DIRECTORY`, `FILE_ATTRIBUTE_NORMAL`), размер, время создания/модификации/доступа, а также серийный номер тома и файловые идентификаторы (File Index / Volume SN).
